...
const uploadConfig: UploadConfig = {
  ...
  allowedTypes: (process.env.ALLOWED_FILE_TYPES || "image/*,application/pdf,text/*").split(","),
...
export async function POST(request: Request) {
  try {
    ...
    if (!contentType.includes("multipart/form-data")) {
      return NextResponse.json({ error: "La requête doit être de type multipart/form-data" }, { status: 400 })
    }
    ...
    const results = await Promise.all(
      files.map(async (file) => {
        ...
        try {
          ...
          console.error(`Erreur lors de l'indexation du fichier ${file.name}:`, indexError)
    ...
    return NextResponse.json({ error: "Une erreur est survenue lors de l'upload des fichiers" }, { status: 500 })
  }
}
